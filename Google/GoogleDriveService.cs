...
public class GoogleDriveService
{
        ...
        string contentType = "application/octet-stream")
    {
        ...
        var fileExists = await FindFileInGoogleDrive(fileName, parentFolderId);
    ...
    {
        string configFolderId = "1nUmMGHn85aBRFtv7pRTqnQseSAxplJgq"; // ✅ Correct config folder ID
        ...
        try
        {
            ...
            return JsonConvert.DeserializeObject<Dictionary<string, TaskMetadata>>(json) ?? new Dictionary<string, TaskMetadata>();
        }
        catch (Exception ex)
        {
...
/// </summary>
   public async Task<List<global::Google.Apis.Drive.v3.Data.File>> ListFilesInFolder(string parentFolderId) 
  {
    ...
    try
    {
        ...
        var request = _driveService.Files.List();
        request.Q = $"'{parentFolderId}' in parents and trashed = false"; // ✅ Filter only non-deleted files
        request.Fields = "files(id, name, mimeType, createdTime)"; // ✅ Fetch useful file details

        var result = await request.ExecuteAsync();
        var files = result.Files?.ToList() ?? new List<global::Google.Apis.Drive.v3.Data.File>(); // ✅ Ensure a List is always returned
    ...
    catch (Exception ex)
    {
        ...
        return new List<global::Google.Apis.Drive.v3.Data.File>(); // ✅ Return an empty list instead of null
    }
}
